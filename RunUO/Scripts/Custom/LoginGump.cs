...
namespace Server.Gumps
{
    public class LoginGump : Gump
    {
        ...
        public LoginGump(Mobile from, PageType pagetype) : base( 0, 0 )
        {
            ...
            AddHtml( 334, 122, 96, 18, @"<basefont color=black size=5>Account Age:</basefont>", false, false );
            AddHtml( 334, 142, 74, 18, @"<basefont color=black size=5>Gametime:</basefont>", false, false );

            TimeSpan age = TimeSpan.FromDays( ( DateTime.Now - account.Created ).Days );
            TimeSpan playtime = account.TotalGameTime;

            AddLabel( 430, 122, 2057, age.Days.ToString() + @" days" );
            AddLabel( 405, 142, 2057, Math.Round(playtime.TotalHours / 24, 1).ToString() + @" days (lifetime)" );
