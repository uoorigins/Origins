	...
	public class Corpse : Container, ICarvable
	{
		...
		public void Carve( Mobile from, Item item )
		{
			...
			else if ( ((Body)Amount).IsHuman && ItemID == 0x2006 )
			{
                ...
                new Torso(m_Owner.Name).MoveToWorld(Location, Map);
                new LeftLeg(m_Owner.Name).MoveToWorld(Location, Map);
                new LeftArm(m_Owner.Name).MoveToWorld(Location, Map);
                new RightLeg(m_Owner.Name).MoveToWorld(Location, Map);
                new RightArm(m_Owner.Name).MoveToWorld(Location, Map);
