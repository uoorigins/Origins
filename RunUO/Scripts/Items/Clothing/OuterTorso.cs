            ...
            else
            {
                if (Effect != ClothEffect.None)
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a fancy dress {0} ({1} charges)", GetEffectString(), Charges)));
                    else
	...
	public class DeathRobe : Robe
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a robe {0} ({1} charges)", GetEffectString(), Charges)));
                    else
	...
	public class PlainDress : BaseOuterTorso
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a plain dress {0} ({1} charges)", GetEffectString(), Charges)));
                    else
